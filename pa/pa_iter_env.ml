...
(* File: pa_iter_env.ml,  by Yoann Padioleau.
...
let mk_rev_bindings _loc fps =
  let coll (i, bindings, patts, vars) fp =
    ...
    let bindings = <:binding< _ = $expr$ and $bindings$ >> in
...
module Generate_sexp_of = struct
  let mk_abst_call _loc tn rev_path =
    <:expr< ($id:Gen.ident_of_rev_path _loc (("" ^ tn) :: rev_path)$ env) >>
  ...
  let sexp_of_td _loc type_name tps rhs =
    ...
    in
    let bnd = Gen.idp _loc ("" ^ type_name) in
    <:binding< $bnd$ = fun env -> $Gen.abstract _loc patts body$ >>
