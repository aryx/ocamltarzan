...
module Generate_sexp_of = struct
  ...
  and sexp_of_tuple _loc tp =
    ...
    let in_expr = <:expr< J.Array $Gen.mk_expr_lst _loc vars$ >> in
  ...
  let rec branch_sum _loc = function
    ...
    | <:ctyp< $uid:cnstr$ of $tps$ >> ->
        ...
        <:match_case<
          $uid:cnstr$ $patt$ ->
            let $bindings$ in
            let args = ( $Gen.mk_expr_lst _loc (vars)$) in
            J.Array ((::) (J.String $str:cnstr$) args)
        >>
  ...
  let sexp_of_record _loc flds_ctyp =
    ...
    let rec coll (patt, expr) = function
      ...
      | _ -> assert false  (* impossible *)
    in
    let init_expr = <:expr< J.Object bnds >> in
